...
async function getBrowserAndPage(id: string): Promise<[Browser, Page]> {
  const browser = await puppeteer.launch({
    ...
    protocolTimeout: 300000, // Increase protocol timeout
  });
  ...
  // Disable JavaScript navigation to prevent redirects
  ...
  console.log('[Puppeteer] Initial page load complete, waiting for hydration...');
  ...
  while (Date.now() - startTime < maxWaitTime) {
    ...
    try {
      const pageState = await page.evaluate(() => {
        ...
        const slides = wrapper ? wrapper.querySelectorAll('[data-speaker-note]') : [];
      ...
      console.log(`[Puppeteer] Check #${checkCount} (${elapsed}s):`, JSON.stringify(pageState, null, 2));

      if (pageState.slideCount > 0) {
        slidesFound = true;
        console.log(`[Puppeteer] ✓ Found ${pageState.slideCount} slides after ${elapsed}s`);
        break;
      }
