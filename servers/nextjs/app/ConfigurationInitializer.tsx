...
export function ConfigurationInitializer({ children }: { children: React.ReactNode }) {
  ...
  // Skip loading screen for export routes
  const isExportRoute = route === '/pdf-maker' || route?.startsWith('/pdf-maker');
  const [isLoading, setIsLoading] = useState(!isExportRoute);
  ...
  const fetchUserConfigState = async () => {
    // Skip configuration check for export-related routes
    const isExportRoute = route === '/pdf-maker' || route?.startsWith('/pdf-maker');
    console.log('[ConfigurationInitializer] Route:', route, 'isExportRoute:', isExportRoute);
    if (isExportRoute) {
      console.log('[ConfigurationInitializer] Skipping config check for export route');
      return;
    }
