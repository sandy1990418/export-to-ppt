...
const PresentationPage = ({ presentation_id }: { presentation_id: string }) => {
  console.log('[PdfMakerPage] Component rendering, id:', presentation_id);
  const { renderSlideContent, loading } = useTemplateLayouts();
  console.log('[PdfMakerPage] useTemplateLayouts loaded, loading:', loading);
  ...
  useEffect(() => {
    console.log('[PdfMakerPage] useEffect triggered, calling fetchUserSlides');
    fetchUserSlides();
  }, []);
  ...
  const fetchUserSlides = async () => {
    try {
      console.log('[PdfMakerPage] Fetching presentation:', presentation_id);
      const data = await DashboardApi.getPresentation(presentation_id);
      console.log('[PdfMakerPage] Presentation data received:', data ? 'success' : 'null', data?.slides?.length, 'slides');
      dispatch(setPresentationData(data));
      setContentLoading(false);
    } catch (error) {
      console.error('[PdfMakerPage] Error fetching user slides:', error);
      setError(true);
