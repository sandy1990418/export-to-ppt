...
def build_public_export_path(file_path: str) -> str:
    """Convert file system path to public URL path for exports"""
    filename = os.path.basename(file_path)
    return f"/app_data/exports/{filename}"
...
async def export_presentation(
    ...
    if export_as == "pptx":
        ...
        return PresentationAndPath(
            ...
            path=build_public_export_path(pptx_path),
        )
    else:
        async with aiohttp.ClientSession() as session:
        ...
        return PresentationAndPath(
            presentation_id=presentation_id,
            path=build_public_export_path(response_json["path"]),
        )
