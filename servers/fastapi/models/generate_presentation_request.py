...
class GeneratePresentationRequest(BaseModel):
    content: Optional[str] = Field(
        default=None, description="The content for generating the presentation"
    )
    slides_markdown: Optional[List[str]] = Field(
        default=None, description="The markdown for the slides (Deprecated, use outlines)"
    )
    outlines: Optional[List[SlideOutlineModel]] = Field(
        default=None, description="The structured outline for the slides"
    )
